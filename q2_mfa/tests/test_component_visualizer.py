...
class TestComponentVisualizer(TestPluginBase):
    ...
    def test_component_visualizer_writes_assets_and_jsonl_payload(self):
        with tempfile.TemporaryDirectory() as output_dir:
            ...
            self.assertIn("'Colorblind Safe'", app_js)
            self.assertIn("Tableau", app_js)
            self.assertIn("Dark2", app_js)
            self.assertIn("Set2", app_js)
            self.assertIn("'component-summary'", app_js)
            self.assertIn("'group-coordinates'", app_js)
            self.assertIn("'partial-sample-coordinates'", app_js)
            self.assertIn("'partial-correlations'", app_js)
            self.assertIn("'partial-contributions'", app_js)
            self.assertIn("Eigenvalues and variance", app_js)
            self.assertIn("function getTableSourceRecords(source)", app_js)
            self.assertIn("function formatTableCellValue(value, column)", app_js)
            self.assertIn("state.featureTableSort = getDefaultTableSort();", app_js)
            self.assertIn("label: `${dimensionLabel(state.xDimension)}", app_js)
            self.assertIn("if (source.aggregate) {", app_js)
            self.assertIn("columns.push({ key: 'aggregate'", app_js)
            self.assertIn(": { column: 'x', direction: 'desc' };", app_js)
            self.assertIn("MAX_FEATURE_OVERLAY_COUNT", app_js)
